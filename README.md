...
Install dependencies with `npm ci` (or `npm install` for local development) and
run the bundled packaging script:

```shell
npm run package
```

This creates `dist/lispbm-language-support-<version>.vsix` which can be
installed in VS Code via **Extensions: Install from VSIX...**.

## Downloading releases

Every published GitHub release automatically includes a packaged `.vsix` file
generated by the release workflow. Head to the repository's
[Releases](https://github.com/laxsjo/lispbm-language-support/releases) page and
download the asset whose name matches the version you want to install.
